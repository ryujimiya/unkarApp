/*
 * unkar 1.6.15
 * Copyright (c) 2008-2010 tn (www.unkar.org)
 */
(function(){
...
unkar = window.unkar = function(url){
	this.nich = {};
	this.data = {
		server	: {},
		board	: [],
...
regs = unkar.regs = {
	...
	search		: /([^<]*)(<[^>]*>)?/g,
	unkar		: /^(www\.)?unkar\.(org|jp)\/read(\/|\.html#|convert\.php\/)(\w+\.2ch\.net|\w+\.bbspink\.com)(\/\w+)(\/\d{9,10})?/
},
...
addEvent = unkar.addEvent = (function(){
...
/*
stopEvent = unkar.stopEvent = function(e){
...
extend = unkar.extend = function(dest, source){
...
addDiv = unkar.addDiv = function(canvas, id){
...
uniq = unkar.uniq = function(array){
	var list = [],
	tmp = [],
	i = 0,
	length = array.length;
		...
		xml.onreadystatechange = function(){
			if(xml.readyState === 4){
				if(xml.status === 200){
					...
					self.contentLength = parseInt((xml.getResponseHeader('Content-Length') || '0'), 10);
					self[func](xml);
...
fn = unkar.prototype = {
	...
	createPopUp: function(){
		this.$popUp = new unkar.popUp(id.popup);
	}
},
	...
	ajaxCall: function(http){
		...
		for(i = 0; i < length; i++){
			if((line = list[i].split(l)).length === 2){
				txt[txt.length] = '<img src="' + img.Convert('folder') + '" alt="ロード中" width="16" height="16"><a href="#' + line[0] + '" onclick="$unkar.access(\'' + line[0] + '\');">' + line[1] + '</a><br>';
			} else {
				txt[txt.length] = '</div><img src="' + img.Convert('folder') + '" alt="ロード中" width="16" height="16"><a href="javascript:$unkar.server.itaView(\'s' + k + '\');" class="tan">' + list[i] + '</a><br><div id="s' + k + '" style="display:none;margin-left:20px;">';
				k++;
			}
		}
		this.printData(txt);
...
ita = fnita.prototype = {
	...
	header: function(txt){
		var start = ['<table><tbody><tr><th><a href="#' + this.url + '" onclick="$unkar.board.sort(\'num\', ' + this.sortflag + ');" class="tan" >No</a></th><th>title</th><th><a href="#' + this.url + '" onclick="$unkar.board.sort(\'res\', ' + this.sortflag + ');" class="tan" >res</a></th><th><a href="#' + this.url + '" onclick="$unkar.board.sort(\'spd\', ' + this.sortflag + ');" class="tan">res/day</a></th><th><a href="#' + this.url + '" onclick="$unkar.board.sort(\'sin\', ' + this.sortflag + ');" class="tan">since</a></th></tr>'];
	...
	style: function(i){
		return '<tr' + ((i % 2) ? ' class="line-color"' : emptyString) + '><td>' + this.data[i].num + '</td><td><a href="#' + this.url + '/' + this.data[i].sin + '" onclick="$unkar.access(\'' + this.url + '/' + this.data[i].sin + '\');">' + this.data[i].thread + '</a></td><td class="res">' + this.data[i].res + '</td><td class="spd">' + this.data[i].spd + '</td><td class="sin">' + this.data[i].since + '</td></tr>';
	},
...
sure = fnsure.prototype = {
	...
	ankerStyle: function(i){
		var color = (this.anker[i] === undefined) ? emptyString : ' onmouseover="$unkar.thread.resPop(' + i + ');" ' + ((this.anker[i].length < 3) ? 'class="ninki"' : 'class="makka"'),
		res = this.res[i];
	...
	anchorPop: function(i){
		...
		$unkar.$popUp.print('a' + i, '<dl>' + this.ankerStyle(i) + '</dl>');
	},
	...
	split: function(str){
		...
		if(!num) return false;
		num = uniq(num);
		$unkar.$popUp.print('c' + num[0], this.resChain(num));
	},

	resPop: function(i){
		$unkar.$popUp.print('r' + i, this.resTree(i));
	},
	
	idPop: function(id){
		$unkar.$popUp.print(id, this.idTree(id));
	},
	...
	ajaxCall: function(http){
		...
		ankerNumberColor = function(str, p1, p2, p3, p4){
			...
			check[p3] = true;
			if(p4){
				return '<a href="#l'+p3+'" onmouseover="$unkar.thread.split(\''+p3+p4+'\');">'+p1+p3+p4+'</a>';
			}
			return '<a href="#l'+p3+'" onmouseover="$unkar.thread.anchorPop('+p3+');">'+p1+p3+'</a>';
		},
		...
		urlLink = function(str, p1, p2){
			...
			} else if(regexp = p2.match(reg.unkar)){
				line = regexp[4] + regexp[5] + (regexp[6] ? regexp[6] : emptyString);
				ret = '<a href="#' + line + '" onclick="$unkar.access(\'' + line + '\');" onmouseover="unkar.title.name(\'' + line + '\');">' + str + '</a>';
			} else {
				ret = hcheck(p1, str);
			}
		...
		idColor = function(str, p1){
			...
			return ' <a href="#l' + i + '"' + color + ' onmouseover="$unkar.thread.idPop(\'' + p1 + '\');">ID:</a>' + p1;
		},
		line = http.responseText
				   .replace(reg.tag, emptyString)
		...
		for(; i < size; i++){
			...
			res[i][3] = res[i][3].replace(reg.id2, '<a href="#l' + i + '" class="tan" onmouseover="$unkar.thread.idPop(\'$1\');">ID:</a>$1')
...
tab = unkar.tab = {
	print: function(name){
		...
		for(; i < length; i++){
			...
			tmp = '<li ondblclick="unkar.menu.tab(' + i + ', \'' + name + '\');" oncontextmenu="unkar.menu.tab(' + i + ', \'' + name + '\', event);"';
			if(url === _url){
				txt[txt.length] = tmp + ' class="now"><a href="#' + url + '" title="' + obj.title + '">' + title + '</a>';
			} else {
				txt[txt.length] = tmp + '><a href="#' + url + '" title="' + obj.title + '" onclick="$unkar.access(\'' + url + '\');">' + title + '</a>';
			}
		}
	...
	delLeft: function(key, name){
		...
		i = 0;
		// 先に表示を変える
		$unkar.hash(list[key].url);
		list[key].print();
		for(; i < key; i++){
			url = list[i].url;
			delete $unkar.nich[url];
		}
		list.splice(0, key);
		this.print(name);
	},
	...
	del: function(key, name){
		...
		this.print(name);
		if(!obj[0]){
			$unkar[name] = undefined;
			$(name).innerHTML = emptyString;
			$unkar.hash(emptyString);
		} else {
			if((url === _url) && obj[--key]){
				$unkar.hash(obj[key].url);
				obj[key].print();
			}
		}
	}
...
menu = unkar.menu = {
	...
	context: function(){
		...
		this.option(nich, 'タブを全て削除', function(){ unkar.tab.delAll(); });
		nich.appendChild(_doc.createElement('hr'));
		this.option(nich, 'スレッドを立てる(p2)', function(){ $unkar.p2Kakiko(id.board); });
		this.option(nich, 'スレッドに書き込む(p2)', function(){ $unkar.p2Kakiko(id.thread); });
		this.option(nich, '板一覧表示', function(){ $unkar.access(unkar.id.server); });
		nich.appendChild(_doc.createElement('hr'));
		this.option(nich, '板一覧更新', function(){ $unkar.renew(unkar.id.server); });
		this.option(nich, 'スレッド一覧更新', function(){ $unkar.renew(unkar.id.board); });
		this.option(nich, 'スレッド更新', function(){ $unkar.renew(unkar.id.thread); });
	...
	tab: function(key, name, e){
		var nich = this.$popUp.plus(true);
		style.change(nich, this.style);
		this.option(nich, 'タブを全て削除', function(e){ unkar.tab.delAll(); });
		this.option(nich, 'このタブの左側を削除', function(e){ unkar.tab.delLeft(key, name); });
		this.option(nich, 'このタブの右側を削除', function(e){ unkar.tab.delRight(key, name); });
		this.option(nich, 'このタブを削除', function(e){ unkar.tab.del(key, name); });
		this.$popUp.point(nich);
	},
	...
	createPopUp: function(){
		this.$popUp = new unkar.popUp('popmenu');
	}
},
...
title = unkar.title = {
	...
	name: function(url){
		...
		if(list[url] === undefined){
			...
			$unkar.$popUp.print(url, list[url]);
		}
	},
	...
	ajaxCall: function(res){
		...
		if(res){
			...
			list[data[0]] = data[1];
			traceLog.stop();
			$unkar.$popUp.print(data[0], data[1]);
		}
	}
},
	...
	printData: function(txt){
		...
		(function(){
			if(dom.ready){
				...
				// スクロール状態を保存する
				if($unkar[name]){
					$unkar[name].scrollkeep();
				}
				element.innerHTML = txt.join(emptyString);
				_doc.title = title + ' - うんかーJS';
				// グローバル変数にオブジェクト格納
				$unkar[name] = self;
				if($unkar.nich[url]){
					// タブを作成
				...
				traceLog.stop(self.contentLength);
			} else {
	...
	setInterval((function(){
		if(browser.msie || browser.opera){
			...
			return function(){
				...
				$unkar.change(ie.location.hash.slice(1));
			};
		} else {
			return function(){
				$unkar.change(location.hash.slice(1));
			};
		}
	})(), conf.change);
	return true;
...
$unkar = window.$unkar = (function(url){
	...
	if(line = url.match(regs.url_split)){
		if(line[3]){
			obj.access(line[1] + line[2]);
		}
		obj.access(id.server);
	}
	return obj;
